...
export default function SignUpScreen({ navigation }) {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      Alert.alert('🎉 Account Created!', `Welcome to NoteX, ${username.trim()}!`,
    ...
    <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <LinearGradient colors={['#00D4AA', '#0ABFBC']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.header}>
          <Animated.View style={[styles.blob, { top: -25, left: -25, width: 120, height: 120 }, { opacity: blob1Opacity, transform: [{ scale: blob1Scale }] }]} />
          <Animated.View style={[styles.blob, { bottom: 0, right: -15, width: 85, height: 85 }, { opacity: blob2Opacity, transform: [{ scale: blob2Scale }] }]} />
        ...
        <Animated.View style={[styles.card, { opacity: cardFade, transform: [{ translateY: cardSlide }] }]}>
          ...
          <Field label="Choose a username" icon="person-outline" value={username} onChangeText={setUsername} autoCapitalize="none" />
          <Field label="Create password" icon="lock-closed-outline" value={password} onChangeText={setPassword} secureTextEntry={!showPass}
            rightIcon={showPass ? 'eye-off-outline' : 'eye-outline'} onRightPress={() => setShowPass(p => !p)} />
          <Field label="Confirm password" icon="shield-checkmark-outline" value={confirm} onChangeText={setConfirm} secureTextEntry={!showConf}
            rightIcon={showConf ? 'eye-off-outline' : 'eye-outline'} onRightPress={() => setShowConf(p => !p)} />
          {error ? (
          ...
          <TouchableOpacity style={styles.btnWrap} activeOpacity={0.88} onPress={handleSignUp} disabled={loading}>
...
const styles = StyleSheet.create({
  ...
  scroll:        { flexGrow: 1 },
  header:        { height: height * 0.30, alignItems: 'center', justifyContent: 'center', paddingBottom: 20, overflow: 'hidden' },
  ...
  footRow:       { flexDirection: 'row', justifyContent: 'center', marginTop: 8 },
  footNote:      { fontSize: 14, color: '#4A7070', fontFamily: 'Nunito_400Regular' },
});
