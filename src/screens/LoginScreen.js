...
// ─────────────────────────────────────────────────────────────────────────────
//  ⚠️  REPLACE the value below with your own Google OAuth Web Client ID.
//  Get one at: https://console.cloud.google.com/
//  Create → OAuth 2.0 Client ID → Web Application
//  Add "https://auth.expo.io" to Authorized redirect URIs
// ─────────────────────────────────────────────────────────────────────────────
...
export default function LoginScreen({ navigation }) {
  ...
  const handleGoogleToken = async (accessToken) => {
    ...
    try {
      ...
      Alert.alert('👋 Welcome!', `Signed in as ${data.username}.`, [
    ...
    <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <LinearGradient colors={['#0ABFBC', '#00D4AA']} start={{ x: 0.1, y: 0 }} end={{ x: 1, y: 1 }} style={styles.header}>
          <View style={[styles.blob, { top: -20, right: -20, width: 90, height: 90 }]} />
          <View style={[styles.blob, { bottom: 10, left: -10, width: 60, height: 60 }]} />
        ...
        <View style={styles.card}>
          ...
          <Field colors={colors} label="Enter your username" icon="person-outline"
            value={username} onChangeText={setUsername} autoCapitalize="none" />
          <Field colors={colors} label="Enter your password" icon="lock-closed-outline"
          ...
          {/* Login button */}
          <TouchableOpacity style={styles.btnWrap} activeOpacity={0.88} onPress={handleLogin} disabled={loading || gLoading}>
...
const styles = StyleSheet.create({
  ...
  scroll:      { flexGrow: 1 },
  header:      { height: height * 0.30, alignItems: 'center', justifyContent: 'center', paddingBottom: 16, overflow: 'hidden' },
  blob:        { position: 'absolute', borderRadius: 999, backgroundColor: 'rgba(255,255,255,0.15)' },
  iconCircle:  { width: 90, height: 90, borderRadius: 45, backgroundColor: 'rgba(255,255,255,0.22)', alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: 'rgba(255,255,255,0.35)', marginBottom: 12 },
